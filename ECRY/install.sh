...
start_write_config() {
    ...
    chmod a+x $installPath/ecry_linux
    if [ -d "/etc/supervisor/conf/" ]; then
        rm /etc/supervisor/conf/ecry.conf -f
        echo "[program:ecry]" >>/etc/supervisor/conf/ecry.conf
        echo "command=${installPath}/ecry_linux" >>/etc/supervisor/conf/ecry.conf
        ...
        echo "command=${installPath}/ecry_linux" >>/etc/supervisor/conf.d/ecry.conf
        ...
        echo "command=${installPath}/ecry_linux" >>/etc/supervisord.d/ecry.ini
...
while :; do
    echo
    echo "-------- ecry 一键安装脚本 by:ethminerproxy--------"
